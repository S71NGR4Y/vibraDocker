...
#This is so the shadows(Mod) show up in logs. 
echo Deleting unused binaries $HACKAZON_PASSWORD
echo $MYSQL_PASSWORD > /mysql-root-pw.txt
echo $HACKAZON_PASSWORD > /ShoppingStreet-db-pw.txt

#set DB password in db.php
sed -i "s/yourdbpass/$HACKAZON_PASSWORD/" /var/www/ShoppingStreet/assets/config/db.php
sed -i "s/youradminpass/$HACKAZON_PASSWORD/" /var/www/ShoppingStreet/assets/config/parameters.php
...
mysql -uroot -p$MYSQL_PASSWORD $HACKAZON_DB < "/var/www/ShoppingStreet/database/createdb.sql"
